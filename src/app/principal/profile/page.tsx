...
export default function PrincipalProfilePage() {
  ...
  const [profile, setProfile] = useState<{ full_name: string; leadership_title: string | null } | null>(null)
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { data } = await supabase.from('profiles').select('full_name, role, leadership_title').eq('id', user.id).single()
        setProfile(data)
      } catch (err) {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{profile?.leadership_title || 'Principal / Vice Principal'} · School Leadership</div>
            </div>
          </div>
        </div>
      </div>
