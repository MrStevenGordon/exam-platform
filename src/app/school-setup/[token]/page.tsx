...
export default function SchoolSetupPage() {
  ...
  useEffect(() => {
    async function verify() {
      const res = await fetch(`/api/school-setup/verify-token?token=${encodeURIComponent(token)}`)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/school-setup/claim', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token, accessToken: signUpData.session.access_token, fullName }),
    })
    const data = await res.json()
  ...
  return (
    <div style={pageWrapperStyle}>
      ...
      <div className="card" style={{ width: '100%', maxWidth: 400, padding: '32px 28px' }}>
        <h1 style={{ marginBottom: 4, fontSize: 18 }}>Create your admin account</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 24 }}>
          You&apos;ll be the first administrator for your school&apos;s portal.
        </p>

        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: 0.5, textTransform: 'uppercase' }}>Full name</label>
            <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required style={{ width: '100%', marginTop: 6 }} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: 0.5, textTransform: 'uppercase' }}>Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required style={{ width: '100%', marginTop: 6 }} />
          </div>
