...
export default function DirectExamResultsPage() {
  ...
  return (
    <div className="page-container" style={{ maxWidth: 520 }}>
      <Link href="/student/tests" style={{ color: 'var(--text-secondary)', fontSize: 14, display: 'block', marginBottom: 16 }}>
        ← Back to tests
      </Link>
      ...
      {result.status === 'completed' && !result.results_released && (
        <div className="card" style={{ textAlign: 'center', padding: 32 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
            Results has not been released yet.
          </p>
        </div>
      )}
      ...
      {result.status === 'completed' && result.results_released && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <Link href={`/student/direct-exam/${examId}/review`} style={{ flex: 1 }}>
              <button className="btn btn-primary" style={{ width: '100%' }}>Review my answers</button>
            </Link>
            <Link href="/student/tests">
              <button className="btn btn-ghost">Back to tests</button>
            </Link>
          </div>
        </div>
      )}
