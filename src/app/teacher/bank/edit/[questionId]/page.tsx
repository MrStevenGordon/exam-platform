    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      <div className="card">
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <MathToolbar textareaId="bank-question-text" value={questionText} onChange={setQuestionText} />
          <textarea id="bank-question-text" value={questionText} onChange={(e) => setQuestionText(e.target.value)} rows={4} style={{ width: '100%', marginTop: 4 }} />
        </div>
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {options.map((opt, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                ...
                <input id={"bank-opt-" + i} value={opt} onChange={(e) => { const u = [...options]; u[i] = e.target.value; setOptions(u) }} style={{ flex: 1 }} />
                <MathSymbolPicker inputId={"bank-opt-" + i} value={opt} onChange={(val) => { const u = [...options]; u[i] = val; setOptions(u) }} />
              </div>
            ))}
            <div style={{ marginTop: 10 }}>
              ...
              <select value={correctAnswer} onChange={(e) => setCorrectAnswer(e.target.value)} style={{ width: '100%', marginTop: 4 }}>
                ...
                {options.filter(Boolean).map((opt, i) => <option key={i} value={opt}>{String.fromCharCode(65 + i)}. {opt}</option>)}
              </select>
            </div>
          </div>
        )}
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {markingPoints.map((mp, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginTop: 6, alignItems: 'center' }}>
                <input value={mp.text} onChange={(e) => { const u = [...markingPoints]; u[i] = { ...u[i], text: e.target.value }; setMarkingPoints(u) }} style={{ flex: 1 }} placeholder="Expected answer" />
                <input type="number" value={mp.marks} onChange={(e) => { const u = [...markingPoints]; u[i] = { ...u[i], marks: parseInt(e.target.value) }; setMarkingPoints(u) }} style={{ width: 60 }} min={1} />
                <button onClick={() => setMarkingPoints(markingPoints.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontWeight: 700 }}>x</button>
              </div>
            ))}
            <button onClick={() => setMarkingPoints([...markingPoints, { text: '', marks: 1 }])} className="btn btn-ghost" style={{ fontSize: 12, marginTop: 8 }}>+ Add marking point</button>
          </div>
        )}
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          <button onClick={handleSave} disabled={saving} className="btn btn-primary">{saving ? 'Saving...' : 'Save changes'}</button>
          <Link href="/teacher/bank"><button className="btn btn-ghost">Cancel</button></Link>
        </div>
      </div>
    </div>
  )
