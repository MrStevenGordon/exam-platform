...
export default function MyClassesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const classIds = teacherClasses.map((tc: any) => tc.class_group_id)
    ...
    <div>
      ...
      {classes.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 32 }}>
          ...
          <a href="/teacher/profile" style={{ color: 'var(--accent-dark)', fontWeight: 700, fontSize: 13 }}>
            Set up my classes →
          </a>
        </div>
      )}
                    ...
                    {expandedClasses.has(cls.id) && (
                      <div style={{ borderTop: '1px solid var(--border)', background: 'var(--page-bg)' }}>
                        {filteredStudents.length === 0 ? (
                          <p style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontSize: 13 }}>No students found.</p>
                        ) : (
                          filteredStudents
