...
export default function DirectExamSessionsPage() {
    ...
    <div style={{ padding: 40, fontFamily: 'sans-serif', maxWidth: 800, margin: '0 auto' }}>
      ...
      {ungradedCount > 0 && (
        <p style={{ background: '#fff3cd', padding: 12, borderRadius: 8 }}>
          {ungradedCount} session(s) still have ungraded essay responses.
        </p>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16 }}>
        {sessions.map((s) => (
          <div key={s.id} style={{ border: '1px solid #ddd', borderRadius: 8, padding: 12, background: s.flagged ? '#fee2e2' : 'white' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <strong>{s.profiles?.full_name || 'Unknown student'}</strong>
              <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 12, background: s.status === 'completed' ? '#d4edda' : '#fff3cd' }}>
                {s.status}
              </span>
            </div>
            {s.status === 'completed' && (
